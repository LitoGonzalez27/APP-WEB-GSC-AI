...
class ResultRepository:
    ...
    def create_result(project_id: int, keyword_id: int, analysis_date: date,
        ...
        if not conn:
            logger.error(f"create_result[ai_mode]({project_id}/{keyword_id}): no DB connection")
            return
        try:
            cur = conn.cursor()
            try:
    ...
    def result_exists_for_date(project_id: int, keyword_id: int, analysis_date: date) -> bool:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"result_exists_for_date[ai_mode]({project_id}/{keyword_id}): no DB connection")
            return False
        try:
            cur = conn.cursor()
            try:
    ...
    def delete_result_for_date(project_id: int, keyword_id: int, analysis_date: date):
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"delete_result_for_date[ai_mode]({project_id}/{keyword_id}): no DB connection")
            return
        try:
            cur = conn.cursor()
            try:
    ...
    def get_project_results(project_id: int, days: int = 30) -> List[Dict]:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"get_project_results[ai_mode]({project_id}): no DB connection")
            return []
        try:
            cur = conn.cursor()
            try:
    ...
    def create_snapshot(project_id: int, snapshot_date: date, metrics: Dict):
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"create_snapshot[ai_mode]({project_id}): no DB connection")
            return
        try:
            cur = conn.cursor()
            try:
