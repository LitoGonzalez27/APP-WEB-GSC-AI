...
@ai_mode_bp.route('/api/projects/<int:project_id>/analyze', methods=['POST'])
@auth_required
...
@ai_mode_bp.route('/api/cron/daily-analysis', methods=['POST'])
...
def _calculate_snapshot_metrics(project_id: int) -> dict:
    ...
    if not conn:
        logger.error(f"_calculate_snapshot_metrics[ai_mode routes]({project_id}): no DB connection")
        return {}
    try:
        cur = conn.cursor()
        try:
