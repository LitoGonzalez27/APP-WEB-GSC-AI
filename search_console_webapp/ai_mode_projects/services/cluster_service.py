...
class ClusterService:
    ...
    def get_project_clusters(project_id: int) -> Dict:
        ...
        if not conn:
            logger.error(f"get_project_clusters[ai_mode]({project_id}): no DB connection")
    ...
    def update_project_clusters(project_id: int, clusters_config: Dict) -> bool:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"update_project_clusters[ai_mode]({project_id}): no DB connection")
            return False
        try:
            cur = conn.cursor()
            try:
    ...
    def get_cluster_statistics(project_id: int, days: int = 30) -> Dict:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"get_cluster_statistics[ai_mode]({project_id}): no DB connection")
            return {
                'enabled': False, 'clusters': [],
                'chart_data': {'labels': [], 'total_keywords': [], 'mentions': []},
                'table_data': [], 'data_freshness': None
            }
        try:
            cur = conn.cursor()
