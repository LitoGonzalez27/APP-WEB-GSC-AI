...
class KeywordRepository:
    ...
    def get_keywords_for_project(project_id: int) -> List[Dict]:
        ...
        if not conn:
            logger.error(f"get_keywords_for_project({project_id}): no DB connection")
            return []
        try:
            cur = conn.cursor()
            try:
    ...
    def get_project_keyword_count(project_id: int) -> int:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"get_project_keyword_count({project_id}): no DB connection")
            return 0
        try:
            cur = conn.cursor()
            try:
    ...
    def delete_keyword(project_id: int, keyword_id: int) -> Dict:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"delete_keyword({project_id},{keyword_id}): no DB connection")
    ...
    def update_keyword(project_id: int, keyword_id: int, new_keyword: str) -> bool:
        ...
        conn = get_db_connection()
        if not conn:
            logger.error(f"update_keyword({project_id},{keyword_id}): no DB connection")
            return False
        try:
            cur = conn.cursor()
            try:
