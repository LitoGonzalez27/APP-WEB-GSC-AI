...
export async function handleDownloadPDF() {
    ...
    try {
        ...
        // PÁGINA 1: Overview + Domain Visibility Over Time + Position Distribution
        ...
        if (btnText) btnText.textContent = 'Page 2/5: Clusters & Competitive...';
        ...
        // Verificar si clusters tiene datos
        ...
        const page2Elements = [];
        if (clustersHasData) page2Elements.push(clustersSection);
        if (competitorsSection) page2Elements.push(competitorsSection);
        
        if (page2Elements.length > 0) {
            if (page2Elements.length === 1) {
                await addSectionToPDF(page2Elements[0], false, 'Competitive Analysis');
            } else {
                const { wrapper: page2Wrapper, originalParents: page2Parents } = createTempWrapper(page2Elements);
                await addSectionToPDF(page2Wrapper, false, 'Clusters & Competitive');
                restoreElements(page2Parents, page2Wrapper);
            }
        }

        // PÁGINA 3: AI Overview Keywords Details
        ...
        // PÁGINA 4: Top Mentioned URLs in AI Overview
