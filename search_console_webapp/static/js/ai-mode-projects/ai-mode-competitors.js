...
export async function updateCompetitors(competitors) {
    ...
    const response = await fetch(`/ai-mode-projects/api/projects/${currentProject.id}/competitors`, {
        method: 'PUT',
        headers: {
...
export async function loadCompetitorsPreview(projectId) {
    ...
    try {
        const response = await fetch(`/ai-mode-projects/api/projects/${projectId}/competitors`);
        if (!response.ok) {
