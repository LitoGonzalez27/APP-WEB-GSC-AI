...
class TopicClusters {
    ...
    handleTermsKeyDown(e) {
        ...
        console.log(`🔍 [CLUSTERS DEBUG] Key pressed: "${e.key}", value: "${value}", clusterName: "${clusterName}"`);

        // Crear cluster al presionar coma o Enter (si hay nombre y términos)
        if ((e.key === ',' || e.key === 'Enter') && value && clusterName) {
            e.preventDefault();
            console.log(`✅ [CLUSTERS] Añadiendo término "${value}" al cluster "${clusterName}"`);
            this.addTermToCurrentCluster(value);
            termsInput.value = '';
        } else if ((e.key === ',' || e.key === 'Enter')) {
            console.log(`❌ [CLUSTERS] No se puede añadir - faltan datos: value="${value}", clusterName="${clusterName}"`);
        }
        ...
        if (e.key === 'Enter' && !value && clusterName) {
            e.preventDefault();
            console.log(`🏁 [CLUSTERS] Finalizando cluster "${clusterName}"`);
