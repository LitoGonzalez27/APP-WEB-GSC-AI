        ...
        const columns = [
            {
                ...
                formatter: (cell, row) => {
                    ...
                    return gridjs.html(`
                        <button 
                            class="serp-view-btn" 
                            onclick="handleSerpView('${keyword.replace(/'/g, "\\'")}')"
                            title="View SERP"
                        >
...
// ================================
// SERP MODAL FUNCTIONS FOR MANUAL AI
// ================================

/**
 * Global function to handle SERP view from Grid.js in Manual AI system
 */
window.handleSerpView = function(keyword) {
    console.log('🔍 Opening SERP modal for keyword in Manual AI:', keyword);
    
    // Get current project ID
    const projectId = window.manualAI?.currentProject?.id || 
                     window.manualAI?.elements?.analyticsProjectSelect?.value;
    
    if (!projectId) {
        alert('Please select a project first');
        console.warn('⚠️ No project selected for SERP view');
        return;
    }
    
    openManualSerpModal(keyword, projectId);
};

/**
 * Open SERP modal specifically for Manual AI projects
 */
function openManualSerpModal(keyword, projectId) {
    const modal = document.getElementById('serpModal');
    if (!modal) {
        console.error('❌ SERP modal element not found');
        alert('Error: SERP modal not available');
        return;
    }
    
    console.log(`🔍 Opening SERP modal for "${keyword}" in project ${projectId}`);
    
    // Update modal content
    const kwSpan = modal.querySelector('#serpKeyword');
    const modalBody = modal.querySelector('.modal-body');
    
    if (kwSpan) kwSpan.textContent = keyword;
    ...
    container.innerHTML = `
        <div class="serp-tabs">
            <button class="serp-tab active" data-tab="quick" aria-selected="true">
                <i class="fas fa-list"></i> Quick View
            </button>
            <button class="serp-tab" data-tab="screenshot" aria-selected="false">
                <i class="fas fa-camera"></i> Screenshot SERP
            </button>
        </div>
        <div class="serp-content">
            <div id="quick-view" class="tab-content active" role="tabpanel">
                <div class="serp-spinner">
                    <i class="fas fa-spinner fa-spin"></i>
                </div>
            ...
            <div id="screenshot-view" class="tab-content" role="tabpanel" style="display:none;">
                <div class="screenshot-info">
                    <p><i class="fas fa-info-circle"></i> The screenshot may take between 5 and 15 seconds to generate.</p>
                    <button class="btn btn-primary load-screenshot" data-keyword="${keyword}" data-project="${projectId}">
    ...
    setupSerpTabs(container);
    
    // Setup close functionality
    setupSerpModalClose(modal);
    
    // Show modal
    modal.classList.add('show');
    modal.style.display = 'block';
    
    // Load quick view data immediately
    loadManualSerpQuickView(keyword, projectId);
}

/**
 * Setup SERP modal tabs
 */
function setupSerpTabs(container) {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            const content = container.querySelector(`#${tabName}-view`);
            if (content) {
    ...
    const screenshotBtn = container.querySelector('.load-screenshot');
    if (screenshotBtn) {
        screenshotBtn.addEventListener('click', () => {
            const keyword = screenshotBtn.dataset.keyword;
            const projectId = screenshotBtn.dataset.project;
            loadManualSerpScreenshot(keyword, projectId);
        });
    }
}

/**
 * Setup SERP modal close functionality
 */
function setupSerpModalClose(modal) {
    // Close button
    const closeBtn = modal.querySelector('.close-btn');
    if (closeBtn) {
        closeBtn.addEventListener('click', () => closeManualSerpModal());
    }
    
    // Click outside to close
    modal.addEventListener('click', (e) => {
        if (e.target === modal) {
            closeManualSerpModal();
        }
    });
...
/**
 * Load SERP quick view data for manual projects
 */
async function loadManualSerpQuickView(keyword, projectId) {
    try {
        console.log(`🔍 Loading SERP quick view for "${keyword}" in project ${projectId}`);
        
        const response = await fetch(`/manual-ai/api/projects/${projectId}/serp?keyword=${encodeURIComponent(keyword)}`);
        const data = await response.json();
        ...
        if (data.success && data.results) {
            displaySerpQuickView(data, quickViewContainer);
        } else {
            quickViewContainer.innerHTML = `
                <div class="serp-error">
                    <i class="fas fa-exclamation-triangle"></i>
                    <p>Failed to load SERP data: ${data.error || 'Unknown error'}</p>
                </div>
            `;
        }
        
    } catch (error) {
        console.error('Error loading SERP quick view:', error);
        const quickViewContainer = document.getElementById('quick-view');
        if (quickViewContainer) {
            quickViewContainer.innerHTML = `
                <div class="serp-error">
                    <i class="fas fa-exclamation-triangle"></i>
                    <p>Error loading SERP data. Please try again.</p>
                </div>
            `;
        }
    }
...
/**
 * Display SERP quick view results
 */
function displaySerpQuickView(data, container) {
    const results = data.results || {};
    const organic = results.organic || [];
    const domain = data.domain || '';
    
    // Find user's domain position
    ...
    let html = `
        <div class="serp-info">
            <div class="serp-meta">
                <h4><i class="fas fa-search"></i> SERP Results for "${data.keyword}"</h4>
                <div class="serp-location">
                    <i class="fas fa-globe"></i> Country: ${data.country_code || 'Unknown'}
                </div>
            </div>
            ...
            ${userPosition ? `
                <div class="user-position-highlight">
                    ...
                    <strong>Your domain found at position #${userPosition}</strong>
                </div>
            ` : `
                <div class="user-position-not-found">
                    <i class="fas fa-search-minus"></i>
                    Your domain not found in top results
                </div>
            `}
        </div>
        
        <div class="serp-results">
            <h5><i class="fas fa-list-ol"></i> Organic Results</h5>
    `;
    
    if (organic.length > 0) {
        organic.slice(0, 10).forEach((result, index) => {
            const position = index + 1;
            const isUserDomain = result.link && result.link.includes(domain);
            
            html += `
                <div class="serp-result ${isUserDomain ? 'user-domain' : ''}">
                    <div class="result-position">${position}</div>
                    <div class="result-content">
                        <h6 class="result-title">${result.title || 'No title'}</h6>
                        <div class="result-url">${result.link || 'No URL'}</div>
        ...
        html += '<p class="no-results">No organic results found</p>';
    }
...
/**
 * Load SERP screenshot for manual projects
 */
async function loadManualSerpScreenshot(keyword, projectId) {
    ...
    try {
        console.log(`📸 Loading SERP screenshot for "${keyword}" in project ${projectId}`);
        
        const response = await fetch(`/manual-ai/api/projects/${projectId}/serp/screenshot?keyword=${encodeURIComponent(keyword)}`);
        const data = await response.json();
        ...
        if (data.success && data.screenshot_url) {
            screenshotContainer.innerHTML = `
                <div class="screenshot-result">
                    <div class="screenshot-meta">
                        <h5><i class="fas fa-camera"></i> SERP Screenshot</h5>
                        <p>Keyword: <strong>${keyword}</strong> | Country: <strong>${data.country_code}</strong></p>
                    </div>
                    <div class="screenshot-image">
                        <img src="${data.screenshot_url}" alt="SERP Screenshot for ${keyword}" loading="lazy">
                    </div>
                </div>
            `;
        } else {
            screenshotContainer.innerHTML = `
                <div class="screenshot-error">
                    <i class="fas fa-exclamation-triangle"></i>
                    <p>Failed to generate screenshot: ${data.error || 'Unknown error'}</p>
                    <button class="btn btn-secondary load-screenshot" data-keyword="${keyword}" data-project="${projectId}">
                        <i class="fas fa-redo"></i> Try Again
                    </button>
                </div>
            `;
            
            // Re-setup button event
            const retryBtn = screenshotContainer.querySelector('.load-screenshot');
            if (retryBtn) {
                retryBtn.addEventListener('click', () => loadManualSerpScreenshot(keyword, projectId));
            }
        }
        
    } catch (error) {
        console.error('Error loading SERP screenshot:', error);
        screenshotContainer.innerHTML = `
            <div class="screenshot-error">
                <i class="fas fa-exclamation-triangle"></i>
                <p>Error generating screenshot. Please try again.</p>
                <button class="btn btn-secondary load-screenshot" data-keyword="${keyword}" data-project="${projectId}">
                    <i class="fas fa-redo"></i> Try Again
                </button>
            </div>
