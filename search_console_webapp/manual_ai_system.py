...
@manual_ai_bp.route('/api/projects/<int:project_id>/serp', methods=['GET'])
@auth_required  
def get_manual_project_serp(project_id):
    """Endpoint SERP específico para proyectos manuales"""
    user = get_current_user()
    ...
    try:
        ...
        # Realizar búsqueda SERP
        serp_results = get_serp_json(params)
        ...
        logger.error(f"Error getting SERP data for project {project_id}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@manual_ai_bp.route('/api/projects/<int:project_id>/serp/screenshot', methods=['GET'])
@auth_required
def get_manual_project_serp_screenshot(project_id):
    """Endpoint para screenshot SERP específico para proyectos manuales"""
    user = get_current_user()
    ...
    try:
        ...
        # Importar servicio SERP
        from services.serp_service import get_page_screenshot
        import os
        
        # Obtener screenshot usando la configuración del proyecto
        api_key = os.getenv('SERPAPI_API_KEY')
        if not api_key:
            return jsonify({
                'success': False,
        ...
        try:
            # Generar screenshot usando el servicio existente
            screenshot_response = get_page_screenshot(
                keyword=keyword,
                site_url_to_highlight=project['domain'],
                api_key=api_key,
                country=internal_country,
                site_url=project['domain']
            )
            
            # Si el screenshot se generó correctamente, retornar la imagen como base64 o URL
            if screenshot_response.status_code == 200:
                import base64
                screenshot_data = screenshot_response.get_data()
            ...
            return jsonify({
                'success': False,
                'error': f'Screenshot generation error: {str(e)}'
            }), 500
        
    except Exception as e:
        logger.error(f"Error getting SERP screenshot for project {project_id}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
