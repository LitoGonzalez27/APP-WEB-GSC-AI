...
@manual_ai_bp.route('/api/setup/create-global-domains-table', methods=['POST'])
@cron_or_auth_required
def create_global_domains_table_endpoint():
    """
    Endpoint temporal para crear la tabla manual_ai_global_domains en producción.
    
    TEMPORAL: Eliminar después de ejecutar una vez en producción.
    """
    try:
        from create_global_domains_table import create_global_domains_table
        success = create_global_domains_table()
        
        if success:
            return jsonify({
                'success': True,
                'message': 'Table manual_ai_global_domains created successfully in production'
            })
        else:
            return jsonify({
                'success': False,
        ...
        logger.error(f"Error creating global domains table: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
